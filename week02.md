# Week 2 Journal

## Task 1 : Created github repo and shared with tutor.
![Github](./images/github.png)

## Task 2 : View Your Computer Information
- Computer Information
![Computer Information](./images/ComputerInfo.png)

## Task 3 : Deploy Linux Web Server in VirtualBox
- Openwrt VM
![Openwrt VM](./images/week2-task4-openwrt.png)

## Task 4 : Browse to OpenWRT Websites
- System Details
![Ram](./images/week2-task4-ram.png)
- After browsing the OpenWRT system, it presents below configuration. 

System Details:
- CPU Architecture: intel Core i5 12th Gen 12600.
- Memory : 106.46 MiB
- Storage : 120.50 MiB
- OS Version : Linux 2.6/3.x/4.x/5.x (64 bit)

